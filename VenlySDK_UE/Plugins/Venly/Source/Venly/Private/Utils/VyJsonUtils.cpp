...
{
	FString StandardizeCase(const FString& StringIn)
	{
	...
	bool JsonObjectToUStruct(const TSharedRef<FJsonObject>& JsonObject, const UStruct* StructDefinition,
		void* OutStruct, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode, FText* OutFailReason)
	{
		if(JsonAttributesToUStruct(JsonObject->Values, StructDefinition, OutStruct, CheckFlags, SkipFlags, bStrictMode, OutFailReason))
		{
			if(StructDefinition->IsChildOf(FVyDTO::StaticStruct()))
			{
				const auto VyDTO = static_cast<FVyDTO*>(OutStruct);
				VyDTO->PostSerialize(JsonObject);
			}

			//if (StructDefinition->HasMetaData("VyPostSerialize"))
			//{
			//	const auto VySerializable = static_cast<FVySerializeable*>(OutStruct);
			//	VySerializable->PostSerialize(JsonObject);
			//}
	...
	bool JsonAttributesToUStruct(const TMap< FString, TSharedPtr<FJsonValue> >& JsonAttributes, const UStruct* StructDefinition, void* OutStruct, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode, FText* OutFailReason)
	{
	...
	bool JsonValueToFPropertyWithContainer(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode, FText* OutFailReason)
	{
		if (!JsonValue.IsValid())
		{
			...
			if (OutFailReason)
			{
				*OutFailReason = FText::FromString(TEXT("Invalid JSON value"));
			}
			return false;
		}
		...
		if (!bJsonArray)
		{
			if (bArrayOrSetProperty)
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::FromString(TEXT("Expecting JSON array"));
				}
				return false;
			}
			...
			if (Property->ArrayDim != 1)
			{
				if (bStrictMode)
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString(TEXT("Property {0} is not an array but has {1} elements")), FText::FromString(Property->GetAuthoredName()), FText::AsNumber(Property->ArrayDim));
					}
					return false;
				}
		...
		if (bStrictMode && (Property->ArrayDim != ArrayValue.Num()))
		{
			...
			if (OutFailReason)
			{
				*OutFailReason = FText::Format(FText::FromString(TEXT("JSON array size is incorrect(has{0} elements, but needs{1})")), FText::AsNumber(ArrayValue.Num()), FText::AsNumber(Property->ArrayDim));
			}
			return false;
		}
	...
	bool JsonAttributesToUStructWithContainer(const TMap< FString, TSharedPtr<FJsonValue> >& JsonAttributes, const UStruct* StructDefinition, void* OutStruct, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode, FText* OutFailReason)
	{
		...
		const bool isDTO = StructDefinition->IsChildOf(FVyDTO::StaticStruct());
		...
		for (TFieldIterator<FProperty> PropIt(StructDefinition); PropIt; ++PropIt)
		{
			...
			{
				const auto VyDTO = static_cast<FVyDTO*>(OutStruct);
				PropertyName = VyDTO->GetJsonPropertyName(Property);
			}
			else PropertyName = StructDefinition->GetAuthoredNameForField(Property);

			
			//if(Property->HasMetaData("PropertyName"))
			//{
			//	PropertyName = Property->GetMetaDataText("PropertyName").ToString();
			//}
			//else PropertyName = StructDefinition->GetAuthoredNameForField(Property);
			...
			if (!JsonValue)
			{
				if (bStrictMode)
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString(TEXT("Missing JSON value named {0}")), FText::FromString(PropertyName));
					}
					return false;
				}
			...
			if (JsonValue->IsValid() && !(*JsonValue)->IsNull())
			{
				...
				{
					if (structProperty->Struct->IsChildOf(FVyOptional::StaticStruct()))
					{
						const auto OptionalProperty = Property->ContainerPtrToValuePtr<FVyOptional>(OutStruct);
						if(!OptionalProperty->SetJsonValue(*JsonValue))
						{
							UE_LOG(LogJson, Error, TEXT("JsonObjectToUStruct - Unable to import JSON value into VyOptional property %s"), *PropertyName);
							if (OutFailReason)
							{
								*OutFailReason = FText::Format(FText::FromString(TEXT("Unable to import JSON value into VyOptional property {0}\n{1}")), FText::FromString(PropertyName), *OutFailReason);
							}
							return false;
						}
					}
					else //todo: refactor...
					{
						...
						if (!JsonValueToFPropertyWithContainer(*JsonValue, Property, Value, ContainerStruct, Container, CheckFlags, SkipFlags, bStrictMode, OutFailReason))
						{
							...
							if (OutFailReason)
							{
								*OutFailReason = FText::Format(FText::FromString(TEXT("Unable to import JSON value into property {0}\n{1}")), FText::FromString(PropertyName), *OutFailReason);
							}
							return false;
						}
					}
	...
	bool UStructToJsonObjectString(const UStruct* StructDefinition, const void* Struct, FString& OutJsonString,
	                               int64 CheckFlags, int64 SkipFlags, int32 Indent, bool bPrettyPrint)
	{
	...
	bool UStructToJsonObject(const UStruct* StructDefinition, const void* Struct, TSharedRef<FJsonObject> OutJsonObject,
	...
	bool UStructToJsonAttributes(const UStruct* StructDefinition, const void* Struct,
		TMap<FString, TSharedPtr<FJsonValue>>& OutJsonAttributes, int64 CheckFlags, int64 SkipFlags)
	{
		...
		const bool isDTO = StructDefinition->IsChildOf(FVyDTO::StaticStruct());
		const FVyDTO* VyDTO = isDTO ? static_cast<const FVyDTO*>(Struct) : nullptr;
		...
		for (TFieldIterator<FProperty> It(StructDefinition); It; ++It)
		{
			...
			if(isDTO)
			{
				VariableName = VyDTO->GetJsonPropertyName(Property);
			}
			else VariableName = StandardizeCase(Property->GetAuthoredName());
			...
			if(const FStructProperty* structProperty = CastField<FStructProperty>(Property))
			{
				if(structProperty->Struct->IsChildOf(FVyOptional::StaticStruct()))
				{
					const auto OptionalProperty = Property->ContainerPtrToValuePtr<FVyOptional>(Struct);
					JsonValue = OptionalProperty->GetJsonValue();
	...
	bool ConvertScalarJsonValueToFPropertyWithContainer(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, const UStruct* ContainerStruct, void* Container, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode, FText* OutFailReason)
	{
		if (FEnumProperty* EnumProperty = CastField<FEnumProperty>(Property))
		{
			if (JsonValue->Type == EJson::String)
			{
				...
				if (IntValue == INDEX_NONE)
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString("Unable to import enum { 0 } from string value{1} for property {2}"), FText::FromString(Enum->CppType), FText::FromString(StrValue), FText::FromString(Property->GetAuthoredName()));
					}
					return false;
				}
		...
		else if (FNumericProperty* NumericProperty = CastField<FNumericProperty>(Property))
		{
			if (NumericProperty->IsEnum() && JsonValue->Type == EJson::String)
			{
				...
				if (IntValue == INDEX_NONE)
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString(TEXT("Unable to import enum {0} from numeric value {1} for property {2}")), FText::FromString(Enum->CppType), FText::FromString(StrValue), FText::FromString(Property->GetAuthoredName()));
					}
					return false;
				}
			...
			else
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::Format(FText::FromString("Unable to import json value into {0} numeric property {1}"), FText::FromString(Property->GetClass()->GetName()), FText::FromString(Property->GetAuthoredName()));
				}
				return false;
			}
		}
		...
		else if (FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Property))
		{
				...
				for (int32 i = 0; i < ArrLen; ++i)
				{
					...
					if (ArrayValueItem.IsValid() && !ArrayValueItem->IsNull())
					{
						if (!JsonValueToFPropertyWithContainer(ArrayValueItem, ArrayProperty->Inner, Helper.GetRawPtr(i), ContainerStruct, Container, CheckFlags & (~CPF_ParmFlags), SkipFlags, bStrictMode, OutFailReason))
						{
							...
							if (OutFailReason)
							{
								*OutFailReason = FText::Format(FText::FromString("Unable to import Array element {0} for property {1}\n{2}"), FText::AsNumber(i), FText::FromString(Property->GetAuthoredName()), *OutFailReason);
							}
							return false;
						}
					}
			...
			else
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::Format(FText::FromString("Unable to import non-array JSON value into Array property {0}"), FText::FromString(Property->GetAuthoredName()));
				}
				return false;
			}
		}
		else if (FMapProperty* MapProperty = CastField<FMapProperty>(Property))
		{
				...
				for (const auto& Entry : ObjectValue->Values)
				{
					if (Entry.Value.IsValid() && !Entry.Value->IsNull())
					{
						...
						if (!JsonValueToFPropertyWithContainer(TempKeyValue, MapProperty->KeyProp, Helper.GetKeyPtr(NewIndex), ContainerStruct, Container, CheckFlags & (~CPF_ParmFlags), SkipFlags, bStrictMode, OutFailReason))
						{
							...
							if (OutFailReason)
							{
								*OutFailReason = FText::Format(FText::FromString("Unable to import Map element {0} key for property {1}\n{2}"), FText::FromString(Entry.Key), FText::FromString(Property->GetAuthoredName()), *OutFailReason);
							}
							return false;
						}
						...
						if (!JsonValueToFPropertyWithContainer(Entry.Value, MapProperty->ValueProp, Helper.GetValuePtr(NewIndex), ContainerStruct, Container, CheckFlags & (~CPF_ParmFlags), SkipFlags, bStrictMode, OutFailReason))
						{
							...
							if (OutFailReason)
							{
								*OutFailReason = FText::Format(FText::FromString("Unable to import Map element {0} value for property {1}\n{2}"), FText::FromString(Entry.Key), FText::FromString(Property->GetAuthoredName()), *OutFailReason);
							}
							return false;
						}
					}
			...
			else
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::Format(FText::FromString("Unable to import non-object JSON value into Map property {0}"), FText::FromString(Property->GetAuthoredName()));
				}
				return false;
			}
		}
		else if (FSetProperty* SetProperty = CastField<FSetProperty>(Property))
		{
				...
				for (int32 i = 0; i < ArrLen; ++i)
				{
					...
					if (ArrayValueItem.IsValid() && !ArrayValueItem->IsNull())
					{
						...
						if (!JsonValueToFPropertyWithContainer(ArrayValueItem, SetProperty->ElementProp, Helper.GetElementPtr(NewIndex), ContainerStruct, Container, CheckFlags & (~CPF_ParmFlags), SkipFlags, bStrictMode, OutFailReason))
						{
							...
							if (OutFailReason)
							{
								*OutFailReason = FText::Format(FText::FromString("Unable to import Set element {0} for property {1}\n{2}"), FText::AsNumber(i), FText::FromString(Property->GetAuthoredName()), *OutFailReason);
							}
							return false;
						}
					}
			...
			else
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::Format(FText::FromString("Unable to import non-array JSON value into Set property {0}"), FText::FromString(Property->GetAuthoredName()));
				}
				return false;
			}
		}
		else if (FTextProperty* TextProperty = CastField<FTextProperty>(Property))
		{
			...
			else if (JsonValue->Type == EJson::Object)
			{
				...
				if (!FJsonObjectConverter::GetTextFromObject(Obj.ToSharedRef(), Text))
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString("Unable to import JSON object with invalid keys into Text property {0}"), FText::FromString(Property->GetAuthoredName()));
					}
					return false;
				}
			...
			else
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::Format(FText::FromString("Unable to import JSON value that is neither string nor object into Text property {0}"), FText::FromString(Property->GetAuthoredName()));
				}
				return false;
			}
		}
		else if (FStructProperty* StructProperty = CastField<FStructProperty>(Property))
		{
			if (StructProperty->Struct->IsChildOf(FVyOptional::StaticStruct()))
			{
				const auto OptionalProperty = Property->ContainerPtrToValuePtr<FVyOptional>(StructProperty->Struct);
				if (!OptionalProperty->SetJsonValue(JsonValue))
				{
					UE_LOG(LogJson, Error, TEXT("JsonValueToUProperty - Unable to import VyOptional Data into %s property %s"), *StructProperty->Struct->GetAuthoredName(), *Property->GetAuthoredName());
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString("Unable to import VyOptional Data into {0} property {1}\n{2}"), FText::FromString(StructProperty->Struct->GetAuthoredName()), FText::FromString(Property->GetAuthoredName()), *OutFailReason);
					}
				}
			}
			else if (JsonValue->Type == EJson::Object)
			{
				...
				if (!JsonAttributesToUStructWithContainer(Obj->Values, StructProperty->Struct, OutValue, ContainerStruct, Container, CheckFlags & (~CPF_ParmFlags), SkipFlags, bStrictMode, OutFailReason))
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString("Unable to import JSON object into {0} property {1}\n{2}"), FText::FromString(StructProperty->Struct->GetAuthoredName()), FText::FromString(Property->GetAuthoredName()), *OutFailReason);
					}
					return false;
				}
			}
			...
			else if (JsonValue->Type == EJson::String && StructProperty->Struct->GetFName() == NAME_DateTime)
			{
				...
				else
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString("Unable to import JSON string into DateTime property {0}"), FText::FromString(Property->GetAuthoredName()));
					}
					return false;
				}
			}
			else if (JsonValue->Type == EJson::String && StructProperty->Struct->GetCppStructOps() && StructProperty->Struct->GetCppStructOps()->HasImportTextItem())
			{
				...
				if (!TheCppStructOps->ImportTextItem(ImportTextPtr, OutValue, PPF_None, nullptr, (FOutputDevice*)GWarn))
				{
					...
					if (Property->ImportText_Direct(ImportTextPtr, OutValue, nullptr, PPF_None) == nullptr)
					{
						...
						if (OutFailReason)
						{
							*OutFailReason = FText::Format(FText::FromString("Unable to import JSON string into {0} property {1}"), FText::FromString(StructProperty->Struct->GetAuthoredName()), FText::FromString(Property->GetAuthoredName()));
						}
						return false;
					}
				}
			...
			else
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::Format(FText::FromString("Unable to import JSON value that is neither string nor object into {0} property {1}"), FText::FromString(StructProperty->Struct->GetAuthoredName()), FText::FromString(Property->GetAuthoredName()));
				}
				return false;
			}
		}
		else if (FObjectProperty* ObjectProperty = CastField<FObjectProperty>(Property))
		{
			...
			else if (JsonValue->Type == EJson::String)
			{
				...
				if (Property->ImportText_Direct(*JsonValue->AsString(), OutValue, nullptr, 0) == nullptr)
				{
					...
					if (OutFailReason)
					{
						*OutFailReason = FText::Format(FText::FromString("Unable to import JSON string into {0} property {1}"), FText::FromString(*ObjectProperty->PropertyClass->GetAuthoredName()), FText::FromString(Property->GetAuthoredName()));
					}
					return false;
				}
			}
		...
		else
		{
			...
			if (Property->ImportText_Direct(*JsonValue->AsString(), OutValue, nullptr, 0) == nullptr)
			{
				...
				if (OutFailReason)
				{
					*OutFailReason = FText::Format(FText::FromString("Unable to import JSON string into property {0}"), FText::FromString(Property->GetAuthoredName()));
				}
				return false;
			}
		}
	...
	TSharedPtr<FJsonValue> ConvertScalarFPropertyToJsonValue(FProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, FProperty* OuterProperty)
	{
		if (FEnumProperty* EnumProperty = CastField<FEnumProperty>(Property))
		{
	...
	bool JsonValueToUProperty(const TSharedPtr<FJsonValue>& JsonValue, FProperty* Property, void* OutValue, int64 CheckFlags, int64 SkipFlags, const bool bStrictMode, FText* OutFailReason)
	{
	...
	TSharedPtr<FJsonValue> UPropertyToJsonValue(FProperty* Property, const void* Value, int64 CheckFlags, int64 SkipFlags, FProperty* OuterProperty)
	{
		if (Property->ArrayDim == 1)
		{

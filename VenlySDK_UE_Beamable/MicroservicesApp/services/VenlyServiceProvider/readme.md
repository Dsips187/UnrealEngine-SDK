...
/project
  ...
  /services
     /VenlyServiceProvider
        VenlyServiceProvider.csproj
        Dockerfile
        Program.cs
        VenlyServiceProvider.cs
     /VenlyServiceProviderLibrary
        VenlyServiceProviderLibrary.csproj
        VenlyServiceProviderLibrary.cs
```

## Running Locally
The VenlyServiceProvider project can be run locally by starting it from an IDE, or by executing the `dotnet run` command.
...
```shell
beam services deploy --ids VenlyServiceProvider
```

### MSBuild Properties
The `VenlyServiceProvider.csproj` file has a few configuration options that can be set.
```xml
<PropertyGroup>
...
#### BeamableTool
By default, both VenlyServiceProvider and VenlyServiceProviderLibrary have the BeamCLI installed as a project tool. 
...
remote swagger. 
```shell
beam project open-swagger VenlyServiceProvider
```
...
at each linked Unity project's `Assets/Beamable/Microservice/Microservices/VenlyServiceProviderClient.cs`. 

Additionally, the `share-code` msbuild target in the VenlyServiceProviderLibrary project will copy their
